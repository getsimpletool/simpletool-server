...
@pytest.mark.asyncio
async def test_210_admin_mcpserver_remove(server_url, admin_auth_token):
    """
    Test removing an MCP server:
    1. Verify the 'time' server exists (created in test_200)
    2. Remove the server via DELETE /public/mcpserver/time
    ...
    async with httpx.AsyncClient() as client:
        ...
        resp = await client.delete(
            f"{server_url}/public/mcpserver/time",
            headers=headers
        )
